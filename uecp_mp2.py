...
parser = argparse.ArgumentParser(description='DVB Stream options')
...
def process_uecp(byte_array) :
    global ebu_chars
    #We may have more than one frame in the byte array, separate them.
    msgs = split_uecp_msgs(byte_array)
    for msg in msgs:
        #Process each UECP packet
        time = datetime.now()
        uecp = unescape_uecp(msg)
...
#Read first MPEG audio packet header to find the format info
